...
# from multitenancy.teams.models import Team, Membership
# from multitenancy.teams.conf import settings
...
#         team_slug = request.environ.get("multitenancy.team")
...
#         else:
#             request.team = None
#         if request.user.is_authenticated and settings.MULTITENANCY_TEAMS_PROFILE_MODEL:
#             if re.search(r"^/teams/[\w-]+/account/signup/", request.path):
#                 return None
#             profiles = settings.multitenancy_TEAMS_PROFILE_MODEL.objects.filter(
#                 user=request.user
#             )
...
#             except settings.multitenancy_TEAMS_PROFILE_MODEL.DoesNotExist:
#                 raise Http404()
#         else:
#             if team_slug is not None:
#                 return check_team_allowed(request)
